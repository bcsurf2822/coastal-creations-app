...
export default function Footer() {
  ...
  // Update footer padding based on footer height
  useEffect(() => {
    const updateFooterSpace = () => {
      // Get footer height if ref is available
      if (footerRef.current) {
        const footerHeight = footerRef.current.offsetHeight;
        // Set a CSS variable for footer height that can be used in the layout
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      const scrollDirection = lastScrollY.current < scrollTop ? "down" : "up";
      lastScrollY.current = scrollTop;

      // Show footer only when scrolling down and near bottom
      // Hide immediately when scrolling up
      if (distanceFromBottom < 300 && scrollDirection === "down") {
        setShowFooter(true);
      } else {
        setShowFooter(false);
      }
