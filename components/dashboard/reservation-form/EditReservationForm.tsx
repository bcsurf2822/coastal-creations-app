...
const EditReservationForm = (): ReactElement => {
  const searchParams = useSearchParams();
  const reservationId = searchParams.get("id");

  const { reservationData, isLoading, error, existingImageUrl } = useReservationData(reservationId);
  ...
  if (!reservationId) {
    return (
      <div className="min-h-screen bg-gray-50 p-4">
        <div className="max-w-4xl mx-auto bg-white rounded-lg shadow p-6">
          <h1 className="text-2xl font-bold text-gray-800 mb-6">Error</h1>
          <p className="text-red-500">No reservation ID provided</p>
          <div className="mt-4">
            <Link
              href="/admin/dashboard"
              className="text-blue-600 hover:underline"
  ...
  return (
    <div className="container mx-auto p-4 sm:p-6 lg:p-8 bg-gradient-to-br from-gray-50 to-gray-100 rounded-xl shadow-lg">
      <ReservationFormBase
        ...
        title="Edit Reservation Program"
        existingImageUrl={existingImageUrl}
        initialData={reservationData || undefined}
      />
    </div>
  );
};
